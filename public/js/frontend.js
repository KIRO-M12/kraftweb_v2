(()=>{'use strict';const dropdownMenus=document.querySelectorAll('.dropdown-menu');const mobileNavTrigger=document.querySelector('.mobile-nav-trigger');const pricingShowMore = document.querySelector(".pricing-show-more");const siteNavContainer=document.querySelector('.site-nav-container');const templatesShowMore=document.querySelector('.templates-show-more');const filterTriggers=document.querySelectorAll('button[data-target]');const toggleClassnameTriggers=document.querySelectorAll('[data-lqd-toggle-class]');const frontendLocalNav=document.querySelector('#frontend-local-navbar');const textRotators=document.querySelectorAll('.lqd-text-rotator');const setAnchors=document.querySelectorAll('[data-set-anchor=true]');let lastActiveTrigger=null;let lastOpenedAccordion=null;
...
return;const clickedOutside=!dd.parentElement.contains(target);if(clickedOutside){dd.classList.remove('show');}});});templatesShowMore?.addEventListener('click',ev=>{ev.preventDefault();const list=document.querySelector('.templates-cards');const overlay=document.querySelector('.templates-cards-overlay');list.style.overflow='visible';list.animate([{maxHeight:'28rem'},{maxHeight:'1500rem'},],{duration:3000,easing:'ease-out',fill:'forwards',});overlay.animate([{opacity:0}],{duration:650,fill:'forwards',easing:'ease-out',});const btnAnima=templatesShowMore.animate([{opacity:0}],{duration:650,fill:'forwards',easing:'ease-out',});btnAnima.onfinish=()=>{overlay.style.visibility='hidden';templatesShowMore.style.visibility='hidden';};});pricingShowMore?.addEventListener("click",e=>{const t=e.target.closest(".pricing-show-more"),o=document.querySelector(".lqd-tabs.text-center"),n=document.querySelector(".pricing-cards-overlay");t&&(e.preventDefault(),o&&(o.style.overflow="visible",o.animate([{maxHeight:"34rem"},{maxHeight:"1500rem"}],{duration:3e3,easing:"ease-out",fill:"forwards"}),n?.animate([{opacity:1},{opacity:0}],{duration:650,fill:"forwards",easing:"ease-out"}),t.animate([{opacity:1},{opacity:0}],{duration:650,fill:"forwards",easing:"ease-out"}).onfinish=()=>{n.style.visibility="hidden",t.style.visibility="hidden"})),console.log("Button container clicked!",e.target)});filterTriggers?.forEach(trigger=>{const targetId=trigger.getAttribute('data-target');const targets=document.querySelectorAll(targetId);const triggerType=trigger.getAttribute('data-trigger-type')||'toggle';if(targets.length<=0){return trigger.setAttribute('disabled',true);}
